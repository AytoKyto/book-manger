...
final class ClaudeRunner
{
    public static function execute(array $run): void
    {
        ...
        try {
            ...
            $styleGuide = BookStorage::readFileAtRelativePath($book['path'], 'style-guide.md');
            $prompt = self::buildPrompt($agent, $chapter, (string) $run['instruction'], $bookContext, $styleGuide);
    ...
    private static function buildPrompt(AgentTemplate $agent, ?array $chapter, string $instruction, string $bookContext, string $styleGuide): string
    {
        $lines = [];

        // Injected directly in the prompt text (not left to the model's discretion to go read
        // the file) so every agent always has it, guaranteed — except the optimizer agent for
        // that exact file, which reads/edits it directly and doesn't need it echoed back.
        ...
        if ($agent->name !== 'style' && trim($styleGuide) !== '') {
            $lines[] = "Guide de style à respecter :";
            $lines[] = trim($styleGuide);
            $lines[] = '';
        }
        ...
        } elseif ($agent->name === 'style') {
            $lines[] = 'Améliore le fichier style-guide.md à la racine du projet.';
            $lines[] = 'Ne modifie aucun autre fichier que style-guide.md.';
        } elseif ($chapter !== null) {
